...
def affine_forward(x, w, b):
    ...
    # reshape input x to N*D
    x_reshape = x.reshape(x.shape[0], -1)
    
    # output = x_reshape*w + b
...
def affine_backward(dout, cache):
    ...
    # dx = dout*w_t, remember to modify the shape
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        # normalize
        x_hat = (x - running_mean.T) / np.sqrt(running_var.T + eps)
...
def layernorm_forward(x, gamma, beta, ln_param):
    ...
    # Calculate mean & variance of input x
...
def layernorm_backward(dout, cache):
    ...
    # Calculate derivatives relative to beta & gamma
    dbeta = np.sum(dout, axis=0) 
...
def max_pool_forward_naive(x, pool_param):
    ...
    S = pool_param['stride']
    H_prime = int((H-HH)/S+1)
    W_prime = int((W-WW)/S+1)
    ...
    for i in range(H_prime):
        for j in range(W_prime):
            selected_x = x[:,:,i*S : i*S+HH, j*S : j*S+WW]
...
def max_pool_backward_naive(dout, cache):
    ...
    for i in range(H_prime):
        for j in range(W_prime):
            ...
            dx[:,:,i*S : i*S+HH, j*S : j*S+WW] += temp_binary * (dout[:,:,i,j])[:,:,None,None]
